...
builder.Services.AddDbContext<DepoStokBitirmeProjesiContext>(options => options.UseSqlServer(connection));
#region Identity User,Role ve DbContext Yapýlandýrmasý ve kullanýcý denetimi 

builder.Services.AddIdentity<AppIdentityUser, AppIdentityRole>().AddEntityFrameworkStores<DepoStokBitirmeProjesiContext>().AddDefaultTokenProviders();
...
#region Cookie Ayarlarýnýn yapýlandýrýlmasý
